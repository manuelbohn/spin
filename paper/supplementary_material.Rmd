...
Developmental trajectories are estimated via a model in which we assume th (that only captures the ME task). 

### Expectations about speaker informativeness

Expectations how rational the speaker is change with age. Older children are assumed to expect
...
All models were implemented in `webppl` [@dippl]. The corresponding model code can be found in the associated online repository (xxxxx). To generate model predictions, we estimated age sensitive parameter distributions for semantic knowledge (by item), speaker informativeness and common ground sensitivity and then passed them through the model in line with the different ways in which they can be combined and aligned. The resulting predictions come in the form of distributions of developmental trajectories for each item in the congruent and the incongruent condition. We compared models based on the full posterior distribution for each model parameter.
