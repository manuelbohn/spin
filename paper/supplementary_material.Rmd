...
P_{S}(u \mid r)\propto P(r\mid u) ^{\alpha_i} \\
P(r\mid u) \propto \mathcal{L}(u, r \mid \theta_{ij})
$$
...
The model with each data-analytic parameter represented explicitly in $\{, \}s$ is:

$$
P_{L}(r \mid u; \{\rho_i, \alpha_i\, \theta_{ij}\})\propto P_{S}(u \mid r; \{\alpha_i, \theta_{ij}\}) \cdot P(r \mid \rho_i) \\ 
...
We consider a model that ignores common ground information, $\rho_i$. This model takes in object specific semantic knowledge and speaker informativeness butuses a prior distribution over objects that is constant across alignment conditions and uniform (e.g., [0.5, 0.5]). This model corresponds to a listener who only focuses on the mutual exclusivity inference and ignores the common ground manipulation. As a consequence, the listener does not differentiate between the two common ground alignment conditions.

#### No mutual exclusivity model

Finally, we consider a model that focuses only on common ground information and ignores the identity of the objects on the tables as well as any inferences their semantic knowledge of the familiar objects license. The predictions of this model correspond to the prior distribution over objects $P(r \mid \rho_i)$. 
...
<!-- \begin{equation}  -->
$$
P^{mixture}_{L}(r \mid u; \{\phi, \rho_i, \alpha_i, \theta_{ij} \}) = \phi \cdot P_{ME}(r \mid u; \{\alpha_i, \theta_{ij}\})  + (1-\phi) \cdot P(r \mid \rho_i) \\
P_{ME}(r \mid u; \{\alpha_i, \theta_{ij}\}) \propto P_{S}(u \mid r; \{\alpha_i, \theta_{ij}\})
$$
where $P_{ME}(r \mid u; \{\alpha_i, \theta_{ij}\})$ represents a mutual-exclusivity inference from a listener with a uniform prior on referents. $P_{S}(u \mid r_{t}$ is the same as in Eq. S above.
<!-- \end{equation}  -->
...
For this model, we make the mixture parameter $\phi$ as a logistic-linear function of age-- $\phi_i = \text{logistic}(\beta^\phi_0 + i \cdot \beta^\phi_1)$ --- and estimate the intercept and slope that yield the best model predictions compared to the data from Experiment 3. The mixture model is then:
$$
P^{dev\_mixture}_{L}(r \mid u; \{\phi_i, \rho_i, \alpha_i, \theta_{ij} \}) = \phi_i \cdot P_{ME}(r \mid u; \{\alpha_i, \theta_{ij}\})  + (1-\phi_i) \cdot P(r \mid \rho_i) \\
$$
