...
p5 <- d %>%
  group_by(experiment,age_num, subid) %>%
  summarise(correct = mean(correct)) 
...
bm_full <- brm(correct ~ age_num * experiment + (1 | subid) + (age_num * experiment | agent), 
...
bm_null <- brm(correct ~ age_num + (1 | subid) + (age_num * experiment | agent), 
          data = model_data, family = bernoulli(),
...
bayes_factor(bm_full, bm_null, log = F, maxiter = 5000)
