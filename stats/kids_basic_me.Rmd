...
```{r}
# import data
d <- read_csv("../data/child_basic_me.csv")%>%
...
ggplot(data = p2, aes(x = age_num, y = correct, col = familiar, fill = familiar)) +
  ...
  geom_smooth(method = "glm", method.args = list(family = "binomial"), se = T, alpha = .5)+
...
ranef_plot <- as_tibble(ranef$familiar, rownames = "familiar")%>%
  ...
  group_by(familiar) %>%
  expand(Estimate.Intercept,Estimate.age_num,grand_intercept,grand_slope,age = d$age_num - mean(d$age_num))%>%
...
data_frame(
  ...
  BF10 = c(round(bf_full_null$bf,2),"-"))%>%
