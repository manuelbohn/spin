...
sem_know_model <- readRDS("../saves/sem_know_model_variant3_100k_log_like.rds")
...
```{r}
p_data_model <- ggplot(data = data_comb, aes(x = age_num, y = correct, col = source, fill = source, lty = alignment)) +
  geom_hline(yintercept = 0.5, lty=2)+
  #geom_jitter(width = 0, height = 0.05, alpha = .6)+
  #geom_smooth(method = "glm", method.args = list(family = "binomial"), se = F, alpha = .5)+
...
var all_log_likes_prag = map(function(row) { 
      var prior = [row.prior, 1-row.prior]
     ...
     var speakerOptimality = speakerOptimalityParameters.intercept  + speakerOptimalityParameters.slope * row.age
        ...
        var pragModelPredictions = pragmaticListener({label: "novel_word"}, prior, speakerOptimality, sem_knowledge)

        return pragModelPredictions.score(row.correct)

    //  query.add(["prediction", item, row.age, row.alignment], ["NA", Math.exp(modelPredictions.score(1))])
  // return loglike
   }, dataFromR.comb_data)


  var log_likelihood_prag = sum(all_log_likes_prag)
  query.add(["log_like", "pragmatics", "NA"], ["NA", log_likelihood_prag])


var all_log_likes_flat = map(function(row) { 
      var prior = [row.prior, 1-row.prior]
        var itemLineParameters = all_item_parameters[row.item]
        var sem_knowledge = logistic(itemLineParameters.intercept + 
            itemLineParameters.slope * row.age)
     ...
     var speakerOptimality = speakerOptimalityParameters.intercept  + speakerOptimalityParameters.slope * row.age
        ...
        var flatModelPredictions = pragmaticListener({label: "novel_word"}, [.5, .5], speakerOptimality, sem_knowledge)

        return flatModelPredictions.score(row.correct)
  ...
  var log_likelihood_flat = sum(all_log_likes_flat)
  query.add(["log_like", "flat_prior", "NA"], ["NA", log_likelihood_flat])
...
#saveRDS(sem_know_model, "../saves/sem_know_model_variant3_100k_log_like.rds")
...
log_like<- sem_know_model %>%
  separate(Parameter, into = c("parameter", "model","age","alignment"), sep = ",")%>%
  filter(parameter == "log_like")%>%
  separate(value, into = c("na", "log_like"),sep = ",")%>%
  select(-na, -age, -alignment)%>%
...
var all_log_likes = map(function(row) { 
      var prior = [row.prior, 1-row.prior]
        ...
        var modelPredictions = pragmaticListener({label: "novel_word"}, [.5,.5], speakerOptimality, sem_knowledge)


    //    display(modelPredictions.score(row.correct))

        return modelPredictions.score(row.correct)

    //  query.add(["prediction", item, row.age, row.alignment], ["NA", Math.exp(modelPredictions.score(1))])
  // return loglike
   }, dataFromR.comb_data)

  var log_likelihood = sum(all_log_likes)
  query.add(["log_like", "NA", "NA"], ["NA", log_likelihood])
...
sem_know_model_flat_prior<- webppl(
  program_code = paste(rsaUtils, rsaModel, flat_prior , sep='\n'),
  data = list(data = me_model_data, comb_data = data_comb_log),
  data_var = "dataFromR",
  model_var = "model",
  chains = 3,
...
saveRDS(sem_know_model_flat_prior, "../saves/sem_know_model_variant3_100k_flat_prior_log_like.rds")


model_pred<- sem_know_model_flat_prior %>%
  separate(Parameter, into = c("parameter", "item","age","alignment"), sep = ",")%>%
  filter(parameter == "prediction")%>%
  separate(value, into = c("na", "me_effect"),sep = ",")%>%
  select(-na)%>%
...
log_like_flat_prior<- sem_know_model_flat_prior %>%
  separate(Parameter, into = c("parameter", "item","age","alignment"), sep = ",")%>%
  filter(parameter == "log_like")%>%
  separate(value, into = c("na", "log_like"),sep = ",")%>%
  select(-na, -item, -age, -alignment)%>%
  mutate(log_like =as.numeric(str_remove(log_like,"[)]")))%>%
  group_by(Chain)%>%
  summarize(logP = logSumExp(log_like))
