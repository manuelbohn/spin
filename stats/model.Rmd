...
x_fb <- sample(1:20000, 50)

plot_model_prag_line_fb <- readRDS("../saves/model_pred_prag_fb.rds")%>%
...
ggplot() +
  geom_hline(yintercept = 0.5, lty=2)+
  geom_jitter(data = data_comb, aes(x = age_num, y = correct), alpha = .1, height = .025, width = 0)+
  geom_line(data = plot_model_prag_line_fb, aes(x=age+2,y = pred, col = alignment, group = interaction(alignment,chain,iteration)),size = .2, alpha = .2)+
  geom_pointrange(data = binned_data, aes(x = subage+0.5, y = data_mean, ymin = data_ci_lower, ymax = data_ci_upper), pch = 5)+
    geom_line(data = binned_data, aes(x = subage+0.5, y = data_mean))+
 # geom_smooth(data = data_comb_plot, aes(x = age_num, y = correct, col = alignment, lty = alignment), method = "glm", method.args = list(family = "binomial"), se = F, alpha = .5, span = 1)+
  labs(x="Age",y="Mutual Exclusivity effect")+
  facet_grid(alignment~item)+
  theme_few() +
  ylim(-0.09,1.09)+
...
x_dmm <- sample(1:length(unique(mixture_dmm$iteration)), 200)

plot_mixture_line <- mixture_dmm%>%
  filter(iteration %in% x_dmm) %>%
  spread(type, value) %>%
  expand_grid(.,age = unique(data_comb$age)) %>%
  mutate(mixture = plogis(intercept + slope * age))

ggplot() +
  geom_line(data = plot_mixture_line, aes(x=age+2,y = mixture, col = factor(chain), group = interaction(chain,iteration)),size = .2, alpha = .1)+
  labs(x="Age",y="Mixture (proportion me inference)")+
  theme_few() +
  ylim(-0.05,1.05)+
  xlim(2,5)+
  guides(alpha = F, fill = F)
...
```{r}
like_chain <- readRDS("../saves/expl_model_likelihood_chain.rds")

model_comp <- readRDS("../saves/expl_model_comparison.rds")%>%
  mutate(BF = format(round(exp(logBF)),scientific=F))
```
...
com_item_params <- bind_rows(
  ...
  item_params_mm%>%mutate(model="mixture"),
  item_params_dmm%>%mutate(model="mixture_dev")
)
...
```{r}
ggsave("../graphs/comparison_item_params.pdf", width = 12, height = 8, scale = 1.5)
```
...
com_global_params <- bind_rows(
  ...
  global_params_dmm%>%mutate(model="mixture_dev")
)
...
```{r}
ggsave("../graphs/global_params_by_model.pdf", width = 8, height = 3, scale = 1.5)
```
...
x_prior <- sample(1:length(unique(com_global_params$iteration)), 200)

plot_prior_line <- com_global_params%>%
  filter(parameter == "prior")%>%
  filter(iteration %in% x_prior) %>%
  spread(type, value) %>%
  expand_grid(.,age = unique(data_comb$age)) %>%
  mutate(prior = plogis(intercept + slope * age))

ggplot() +
  geom_line(data = plot_prior_line, aes(x=age+2,y = prior, col = model, group = interaction(chain,iteration)),size = .2, alpha = .1)+
  labs(x="Age",y="Prior")+
  theme_few() +
  facet_grid(~model)+
  ylim(-0.05,1.05)+
...
com_global_sigmas <- bind_rows(
  item_sigma%>%mutate(model="pragmatic"),
  item_sigma_fb%>%mutate(model="fullBayes"),
  item_sigma_mm%>%mutate(model="mixture"),
  item_sigma_dmm%>%mutate(model="mixture_dev")
)

ggplot(com_global_sigmas, aes(x = value, fill = model, group = paste(model,chain)))+
  geom_density(alpha = 0.3)+
  xlab("Parameter")+
