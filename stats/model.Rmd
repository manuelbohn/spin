...
var model  = function(){
  ...
  var params = map(function(cndtn){
    ...
    return {sem_int: sem_int, sem_slope: sem_slope, familiar: cndtn, so_int: so_int, so_slope: so_slope}
...
#   chains = 4,
#   cores = 4,
#   inference_opts = list(method = "MCMC", samples = 900, burn = 300, verbose = T)
# )
#  
...
sem_know_model<- webppl(
  ...
  chains = 1,
  cores = 1,
  inference_opts = list(method = "MCMC", samples = 5, burn = 1, verbose = T)
)
...
sem_know <- sem_know_model %>%
  select(value) %>%
  map_df(bind_rows)%>%
  rename(sem_int = `0.sem_int`,
         sem_slope = `0.sem_slope`,
         familiar = `0.familiar`,
         so_int = `0.so_int`,
         so_slope = `0.so_slope`)%>% 
...
plot_sem_know <- sem_know_params%>%
  ...
  expand(familiar, sem_slope, sem_int,age = me_model_data$age)%>%
  mutate(sem_know = plogis(sem_int + sem_slope * age))%>%
...
model_plot <- ggplot(age_me_effect, aes(x=age,y = me_effect, col = familiar))+
