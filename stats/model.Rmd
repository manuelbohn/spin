...
plot_model_pred <- model_pred%>%
  ...
  group_by(run, model, alignment, item, age)%>%
...
ggplot() +
  ...
  geom_line(data = plot_model_pred, aes(x=age+2,y = mean, col = run), size = 1)+
  geom_ribbon(data = plot_model_pred, aes(x = age+2, ymin = ci_lower, ymax = ci_upper, fill = run), alpha = .4) +
...
length(unique(model_pred$iteration))

x <- sample(1:10000, 500)

plot_model_prag__line <- model_pred%>%
  filter(model == "pragmatic", iteration %in% x)

ggplot() +
  geom_hline(yintercept = 0.5, lty=2)+
  geom_line(data = plot_model_prag__line, aes(x=age+2,y = pred, col = alignment, group = interaction(chain,iteration)),size = .2, alpha = .05)+
...
plot_cor_model_pred <- cor_model_pred %>%
  left_join(
    bind_rows(
      binnned_data%>%mutate(run = "no re"),
      binnned_data%>%mutate(run = "re"),
