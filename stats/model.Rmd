...
# load this for model parameters
...
#saveRDS(sem_know_model, "../saves/sem_know_new_model_3.rds")
...
```{r}
ggplot(data = data_comb, aes(x = age_num, y = correct, col = source, fill = source)) +
  geom_hline(yintercept = 0.5, lty=2)+
  #geom_jitter(width = 0, height = 0.05, alpha = .6)+
  #geom_smooth(method = "glm", method.args = list(family = "binomial"), se = F, alpha = .5)+
  geom_smooth(method = "loess", se = T, alpha = .5, span = 2)+
...
```{r}
#ggsave("../graphs/combination_model_data_by_item.pdf", width = 8, height = 2, scale = 1.5)
```
