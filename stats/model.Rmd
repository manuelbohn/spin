...
#subject_sigma <- readRDS("../saves/subject_sigma.rds")
...
saveRDS(log_like_global, "../saves/log_like_global.rds")


comp <- bind_rows(
  readRDS("../saves/log_like_prag.rds")%>%mutate(id = 1:length(log_like)), 
  readRDS("../saves/log_like_global.rds")%>%mutate(id = 1:length(log_like))
)%>%
...
log_like_flat <- data_comb %>%
  ...
  select(chain,iteration,age, alignment,item,log_like_flat)

gc()

log_like_prior <- data_comb %>%
  select(-age)%>%
  mutate(age = age_month,
         item = as.character(item))%>%
  left_join(readRDS("../saves/model_pred_prior.rds"))%>%
  mutate(like = ifelse(correct == 1, pred, 1 - pred),
         log_like_prior = log(like))%>%
  select(chain,iteration,age, alignment,item,log_like_prior)

gc()

prag_vs_global <- log_like_prag%>%
  ...
  saveRDS(.,"../saves/model_comp_prag_vs_global.rds")
...
%>%
  summarise(logBF = logSumExp(prag_vs_global))
