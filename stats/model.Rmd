...
var all_log_likes = map(function(row) { 
  ...
  query.add(["pragmaticModelPredictions","parameterFree", row.condition, row.familiarObject, row.subid], [row.age, Math.exp(pragModelPredictions.score(1))])  


  var noisyPragModelPredictions = addNoise(pragModelPredictions, noise)
  
  query.add(["pragmaticModelPredictions","noise", row.condition, row.familiarObject, row.subid], [row.age, Math.exp(noisyPragModelPredictions.score(1))])
...
sem_know_model<- webppl(
  ...
  chains = 4,
  cores = 4,
  packages = c("utils/webppl-sample-writer")
)



#saveRDS(sem_know_model, "../saves/sem_know_model_variant3_100k_model_comparison_noise.rds")

sem_know_model <- readRDS("../saves/sem_know_model_variant3_200k_model_comparison_noise.rds")

model <- bind_rows(
  read_csv("../saves/model/samples126.csv")%>%mutate(chain = 1),
  read_csv("../saves/model/samples551.csv")%>%mutate(chain = 2),
  read_csv("../saves/model/samples661.csv")%>%mutate(chain = 3),
  read_csv("../saves/model/samples950.csv")%>%mutate(chain = 4)
)
...
log_like <- model %>%
  ...
  rename(type = a, 
         ...
         parameter = c, 
         log_like = e)%>%
  mutate(log_like = as.numeric(log_like))

log_like_chain<- sem_know_model %>%
