...
data_comb %>%
  ...
  select(chain, iteration, age, alignment,item,log_like_prag)%>%
  saveRDS(.,"../saves/log_like_prag")

log_like_global <- data_comb %>%
  select(-age)%>%
  mutate(age = age_month,
         item = as.character(item))%>%
  left_join(model_pred_global)%>%
  mutate(like = ifelse(correct == 1, pred, 1 - pred),
         log_like_global = log(like))%>%
  select(chain,iteration, age, alignment,item,log_like_global)

log_like_flat <- data_comb %>%
  select(-age)%>%
  mutate(age = age_month,
         item = as.character(item))%>%
  left_join(model_pred_flat)%>%
  mutate(like = ifelse(correct == 1, pred, 1 - pred),
