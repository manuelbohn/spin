...
run_comp <- sem_know_run %>%
  ...
  group_by(run,item, age) %>%
  summarise(sem_know = estimate_mode(sem_know), 
            sem_know_uci = hdi_upper(sem_know_ci),
            sem_know_lci = hdi_lower(sem_know_ci))

ggplot(run_comp, aes(x = age+2, y= sem_know, fill = run))+
  geom_line(size = 1, aes(col = run))+
  geom_ribbon(aes(ymin=sem_know_lci, ymax = sem_know_uci), alpha = .2)+
  ylab("Semantic knowledge")+
  xlab("Age")+
  ylim(0,1)+
  facet_wrap(~item)+
