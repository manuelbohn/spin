...
```{r}
ggplot(log_like_model_2_free, aes(x = model, y = logP, col = model))+
...
```{r}
ggsave("../graphs/like_talk_free.png", width = 4, height = 2.5, scale = 1.5)
```
...
subject_sigma <- readRDS("../saves/subject_sigma_re.rds")


item_params <- bind_rows(
  readRDS("../saves/item_params.rds")%>%mutate(run = "no re"),
  readRDS("../saves/item_params_re.rds")%>%mutate(run = "re")
)

global_params <- bind_rows(
  readRDS("../saves/global_params.rds")%>%mutate(run = "no re"),
  readRDS("../saves/global_params_re.rds")%>%mutate(run = "re")
)


item_sigma <- bind_rows(
  readRDS("../saves/item_sigmas.rds")%>%mutate(run = "no re"),
  readRDS("../saves/item_sigma_re.rds")%>%mutate(run = "re")
)


model_pred <- bind_rows(
    readRDS("../saves/model_pred.rds")%>%mutate(run = "no re")%>%filter(model == "pragmatic"),
    readRDS("../saves/model_pred_re.rds")%>%mutate(run = "re")%>%filter(model == "pragmatic")
)


x <- readRDS("../saves/sem_know_model_variant3_200k_model_comparison_noise.rds")
...
global_params_summary <- global_params %>%
  group_by (run,parameter,type)%>%
  summarise(mode = estimate_mode(value),
            uci = hdi_upper(value),
            lci = hdi_lower(value))


subject_sigma_summary <- subject_sigma %>%
  group_by (parameter)%>%
  summarise(mode = estimate_mode(value),
            uci = hdi_upper(value),
            lci = hdi_lower(value))
...
plot_item_params <- ggplot(item_params, aes(x = value, fill = factor(chain)))+
  ...
  xlab("Parameter")+
  facet_grid(run ~parameter + item, scales = 'free')+
  theme_few()
...
plot_global_params <- ggplot(global_params, aes(x = value, fill = factor(chain)))+
  geom_density(alpha = 0.3)+
  xlab("Parameter")+
  facet_grid(run~type + parameter, scales = 'free')+
  theme_few()
...
```{r}
plotg_subj_sigma <- ggplot(subject_sigma, aes(x = value, fill = factor(chain)))+
  geom_density(alpha = 0.3)+
  xlab("Subject Sigma")+
  theme_few()
...
x <- sample(1:length(unique(model_pred_prag$iteration)), 100)
...
ggplot() +
  geom_hline(yintercept = 0.5, lty=2)+
  geom_line(data = plot_model_prag_line, aes(x=age+2,y = pred, col = alignment, group = interaction(chain,iteration)),size = .2, alpha = .05)+
  geom_smooth(data = data_comb_plot, aes(x = age_num, y = correct), col = "black", method = "glm", method.args = list(family = "binomial"), se = T, alpha = .5, span = 2)+
  labs(x="Age",y="Mutual Exclusivity effect")+
  facet_grid(alignment~item)+
  theme_few() +
  ylim(-0.05,1.05)+
...
### Global pragmatic model

```{r}

model_pred_global<- readRDS("../saves/model_pred_global.rds")

plot_model_pred_global <- model_pred_global%>%
  group_by(alignment, item, age)%>%
  summarise(mean = estimate_mode(pred),
            ci_lower = hdi_lower(pred),
            ci_upper = hdi_upper(pred))
...
ggplot() +
  geom_hline(yintercept = 0.5, lty=2)+
  geom_smooth(data = data_comb_plot, aes(x = age_num, y = correct), col = "black", method = "loess", se = T, alpha = .5, span = 2)+
  geom_line(data = plot_model_pred_global, aes(x=age+2,y = mean, col = alignment), size = 1)+
  geom_ribbon(data = plot_model_pred_global, aes(x = age+2, ymin = ci_lower, ymax = ci_upper, fill = alignment), alpha = .4) +
  labs(x="Age",y="Mutual Exclusivity effect")+
  facet_grid(alignment~item)+
  theme_few() +
  ylim(-0.05,1.05)+
...
# cor_prag_chain <- model_pred_prag%>%
#   mutate(age = as.numeric(age) + min(data_comb$age_num),
#          age = cut(age, 
#                       breaks = c(2,3,4,5),
#                       labels = c(2,3,4)))%>%
...
model_pred_flat<- readRDS("../saves/model_pred_flat.rds")

cor_flat <- model_pred_flat%>%
  mutate(age = as.numeric(age) + min(data_comb$age_num),
         age = cut(age, 
                      breaks = c(2,3,4,5),
                      labels = c(2,3,4)))%>%
...
model_pred_prior<- readRDS("../saves/model_pred_prior.rds")

# cor_prior <- model_pred_prior%>%
#   mutate(age = as.numeric(age) + min(data_comb$age_num),
#          age = cut(age, 
#                       breaks = c(2,3,4,5),
#                       labels = c(2,3,4)))%>%
...
plot_cor_model_pred <- cor_model_pred %>%
  left_join(
    bind_rows(
      # binnned_data%>%mutate(model = "pragmatic", chain = 1),
      # binnned_data%>%mutate(model = "pragmatic", chain = 2),
      # binnned_data%>%mutate(model = "pragmatic", chain = 3),
      # binnned_data%>%mutate(model = "pragmatic", chain = 4),
      # binnned_data%>%mutate(model = "pragmatic", chain = 5),
      # binnned_data%>%mutate(model = "pragmatic", chain = 6)
...
plot_model_pred_flat <- model_pred_flat%>%
  group_by(alignment, item, age)%>%
  summarise(mean = estimate_mode(pred),
            ci_lower = hdi_lower(pred),
            ci_upper = hdi_upper(pred))
...
log_like_prag <- data_comb %>%
  select(-age)%>%
  mutate(age = age_month)%>%
  left_join(plot_model_pred_prag)%>%
  mutate(like = ifelse(correct == 1, mean, 1 - mean),
         log_like_prag = log(like))%>%
  select(age, alignment,item,log_like_prag)

log_like_global <- data_comb %>%
  select(-age)%>%
  mutate(age = age_month)%>%
  left_join(plot_model_pred_global)%>%
  mutate(like = ifelse(correct == 1, mean, 1 - mean),
         log_like_global = log(like))%>%
  select(age, alignment,item,log_like_global)

log_like_flat <- data_comb %>%
  select(-age)%>%
  mutate(age = age_month)%>%
  left_join(plot_model_pred_flat)%>%
  mutate(like = ifelse(correct == 1, mean, 1 - mean),
         log_like_flat = log(like))%>%
  select(age, alignment,item,log_like_flat)


model_comp <- log_like_prag%>%
  left_join(log_like_global)%>%
  left_join(log_like_flat)%>%
  mutate(prag_vs_global = log_like_prag - log_like_global,
         prag_vs_flat = log_like_prag - log_like_flat)%>%
  na.omit(prag_vs_global,prag_vs_flat)%>%
  summarize(prag_vs_global = logSumExp(prag_vs_global),
            prag_vs_flat = logSumExp(prag_vs_flat))
...
model_comp <- log_like_2%>%
  ...
  gather(model,logP,-noise)
  

log_like_model_2_free <- log_like_model_2%>%
  filter(noise == "parameter_free")
