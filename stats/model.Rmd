...
model_pred <- sem_know_model %>%
  separate(Parameter, into = c("type", "token"))%>%
...
params<- sem_know_model %>%
  separate(Parameter, into = c("parameter", "type"), sep = ",")%>%
  filter(parameter == "parameter")%>%
  separate(value, into = c("slope", "int"),sep = ", ")%>%
  mutate(slope = as.numeric(str_remove_all(slope, "[(c]")),
         int = as.numeric(str_remove(int, "[)]")))%>%
  gather(parameter, value, int, slope)


sem_know_params <- params %>%
  filter(type != "speaker_optimality",
         type != "global_sem",
         type != "global_sem_sigmas")%>%
  rename(item = type)%>%
...
ggplot(param_params, aes(x = value))+#, fill = factor(Chain)))+
  geom_density(alpha = 0.3)+
  xlab("Parameter")+
  facet_grid(parameter~type, scales = 'free')+
  theme_few()
...
```{r}
sigma_params <- params %>%
  filter(type == "global_sem_sigmas")

ggplot(sigma_params, aes(x = value))+#, fill = factor(Chain)))+
  geom_density(alpha = 0.3)+
  xlab("Sigma")+
...
inv_model_plot <- ggplot(model_pred, aes(x=age+2,y = pred, col = item))+
