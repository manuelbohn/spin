...
# Model (nesting within participant)
...
data_comb_log <- read_csv("../data/combination.csv") %>%
  filter(condition != "train")%>%
  mutate(alignment = condition,
         item = familiarObject, 
...
var all_log_likes = map(function(row) { 
  ...
  var noisyPriorOnlyModelPredictions = addNoise(priorOnlyModelPredictions, noise) 
    ...
    query.add(["modelPrediction","pragmatic","noise", row.condition, row.familiarObject, row.subid], [row.age, Math.exp(noisyPragModelPredictions.score(1))])
  ...
  query.add(["modelPrediction","globalPrag","noise", row.condition, row.familiarObject, row.subid], [row.age, Math.exp(noisyGlobalPragModelPredictions.score(1))])  
...
sem_know_model<- webppl(
  ...
  data = list(data = me_model_data, comb_data = data_comb_log),
  data_var = "dataFromR",
  model_var = "model",
  chains = 2,
  cores = 2,
...
read_csv("../saves/model/samples434.csv")%>%mutate(chain =1)%>%
  saveRDS("../saves/model/chain_1.rds")

read_csv("../saves/model/samples434.csv")%>%mutate(chain =2)%>%
  saveRDS("../saves/model/chain_2.rds")

read_csv("../saves/model/samples434.csv")%>%mutate(chain =1)%>%
  saveRDS("../saves/model/chain_3.rds")

read_csv("../saves/model/samples434.csv")%>%mutate(chain =2)%>%
  saveRDS("../saves/model/chain_4.rds")

read_csv("../saves/model/samples434.csv")%>%mutate(chain =1)%>%
  saveRDS("../saves/model/chain_5.rds")

read_csv("../saves/model/samples434.csv")%>%mutate(chain =2)%>%
  saveRDS("../saves/model/chain_6.rds")

read_csv("../saves/model/samples434.csv")%>%mutate(chain =1)%>%
  saveRDS("../saves/model/chain_7.rds")

read_csv("../saves/model/samples434.csv")%>%mutate(chain =2)%>%
  saveRDS("../saves/model/chain_8.rds")

read_csv("../saves/model/samples434.csv")%>%mutate(chain =2)%>%
  saveRDS("../saves/model/chain_9.rds")

read_csv("../saves/model/samples434.csv")%>%mutate(chain =2)%>%
  saveRDS("../saves/model/chain_10.rds")


sem_know_model <- bind_rows(
  readRDS("../saves/model/chain_1.rds"),
  readRDS("../saves/model/chain_2.rds"),
  readRDS("../saves/model/chain_3.rds"),
  readRDS("../saves/model/chain_4.rds"),
  readRDS("../saves/model/chain_5.rds"),
  readRDS("../saves/model/chain_6.rds"),
  readRDS("../saves/model/chain_7.rds"),
  readRDS("../saves/model/chain_8.rds"),
  readRDS("../saves/model/chain_9.rds"),
  readRDS("../saves/model/chain_10.rds")
)

saveRDS(sem_know_model, "../saves/model_complete.rds")

```
## Model comparison

```{r}
log_like <- sem_know_model %>%
  filter(a == "log_like")%>%
  select(b,c,e)%>%
  rename(model = b, 
         parameter = c,
...
saveRDS(log_like, "../saves/log_like.rds")
log_like <- readRDS("../saves/log_like.rds")
...
saveRDS(log_like_model, "../saves/log_like_model.rds")
log_like_model <- readRDS("../saves/log_like_model.rds")
...
saveRDS(log_like_chain, "../saves/log_like_chain.rds")
log_like_chain <- readRDS("../saves/log_like_chain.rds")

log_like_chain %>%
  group_by(model)%>%
  mutate(noise = round(noise,2),
         parameter_free = round(parameter_free,2))%>%
  summarise(range_noise = paste(range(noise)[1],range(noise)[2], sep = " / "))
```
...
model_params<- sem_know_model %>%
  filter(a == "parameter")

saveRDS(model_params, "../saves/model_params.rds")
model_params <- readRDS("../saves/model_params.rds")
...
global_sigma <- readRDS("../saves/global_sigma.rds")
...
ggplot() +
  ...
  geom_line(data = plot_model_pred, aes(x=age+2,y = mean, col = model), size = 1)+
  geom_ribbon(data = plot_model_pred, aes(x = age+2, ymin = ci_lower, ymax = ci_upper, fill = model), alpha = .4) +
...
noise_plot <- bind_rows(
  ...
  global_prag_model_noise %>%
    filter(Parameter %in% c("noise"))%>%
    mutate(Model = "Pragmatic no word knowledge"),
  flat_model_noise %>%
    filter(Parameter %in% c("noise"))%>%
