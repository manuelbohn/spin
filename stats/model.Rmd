...
var model  = function(){
  ...
  var speakerOptimalityParameters = {
    ...
    slope: uniformDrift({a: 0, b: 1, width: 0.5})
  }
...
# 3 chains, 6000 iterations per chain, 2000 burn in, lag = 1
sem_know_model <- readRDS("../saves/sem_know_model.rds")
# 8 chains, 4000 iterations per chain, 2000 burn in, lag = 1
sem_know_model <- readRDS("../saves/sem_know_model_2.rds")

sem_know_model <- sem_know_model %>%
  filter(Chain != "1")
```
...
global_sigma<- sem_know_model %>%
  separate(Parameter, into = c("parameter", "type","token"), sep = ",")%>%
  filter(parameter == "parameter", type == "sigma")%>%
...
```{r}
ggplot(item_params, aes(x = value, fill = factor(Chain)))+
...
ggplot(global_params, aes(x = value, fill = factor(Chain)))+
  geom_density(alpha = 0.3)+
  xlab("Parameter")+
...
```{r}
ggplot(global_sigma, aes(x = value, fill = factor(Chain)))+
  geom_density(alpha = 0.3)+
  xlab("Sigma")+
...
# 3 chains, 6000 iterations per chain, 2000 burn in, lag = 1
sem_know_run_1 <- readRDS("../saves/sem_know_model.rds")%>%
  separate(Parameter, into = c("parameter", "type","item"), sep = ",")%>%
  filter(parameter == "parameter", type == "items")%>%
  separate(value, into = c("int", "slope"),sep = ", ")%>%
  mutate(int = as.numeric(str_remove_all(int, "[(c]")),
...
# 8 chains, 4000 iterations per chain, 2000 burn in, lag = 1
sem_know_run_2 <- readRDS("../saves/sem_know_model_2.rds")%>%
  separate(Parameter, into = c("parameter", "type","item"), sep = ",")%>%
  filter(parameter == "parameter", type == "items")%>%
  separate(value, into = c("int", "slope"),sep = ", ")%>%
  mutate(int = as.numeric(str_remove_all(int, "[(c]")),
...
sem_know_run_iteration <- bind_rows(
   sem_know_run_1,
   sem_know_run_2)

sem_know_run_summary <- sem_know_run_iteration%>%
  group_by(run,Chain,item)%>%
  summarise(overall_int = estimate_mode(int),
            overall_slope = estimate_mode(slope))


sem_know_run <- sem_know_run_iteration %>%
  left_join(sem_know_run_summary)

run_comp <- sem_know_run %>%
  left_join(expand.grid(
      age = unique(me_model_data$age),
      item = unique(me_model_data$familiar)), .) %>%
  mutate(sem_know_ci = plogis(int + slope * age),
         sem_know = plogis(overall_int + overall_slope * age)) %>%
  group_by(run,Chain,item, age) %>%
  summarise(sem_know = estimate_mode(sem_know), 
            sem_know_uci = hdi_upper(sem_know_ci),
            sem_know_lci = hdi_lower(sem_know_ci))

ggplot(run_comp, aes(x = age+2, y= sem_know, fill = run, group = paste(run,Chain)))+
  geom_line(size = .3, aes(col = run))+
  geom_ribbon(aes(ymin=sem_know_lci, ymax = sem_know_uci), alpha = .1)+
  ylab("Semantic knowledge")+
  xlab("Age")+
  ylim(0,1)+
  facet_grid(item~run)+
...
ggsave("../graphs/model_by_chain_run.pdf", width = 3, height = 12, scale = 1.5)
```
...
# 3 chains, 6000 iterations per chain, 2000 burn in, lag = 1
so_run_1 <- readRDS("../saves/sem_know_model.rds")%>%
  separate(Parameter, into = c("parameter", "type","token"), sep = ",")%>%
  filter(parameter == "parameter", type == "parameters", token == "speaker_optimality")%>%
  separate(value, into = c("int", "slope"),sep = ", ")%>%
  mutate(int = as.numeric(str_remove_all(int, "[(c]")),
         slope = as.numeric(str_remove(slope, "[)]")))%>%
  mutate(run = "1")
...
# 8 chains, 4000 iterations per chain, 2000 burn in, lag = 1
so_run_2 <- readRDS("../saves/sem_know_model_2.rds")%>%
  separate(Parameter, into = c("parameter", "type","token"), sep = ",")%>%
  filter(parameter == "parameter", type == "parameters", token == "speaker_optimality")%>%
  separate(value, into = c("int", "slope"),sep = ", ")%>%
  mutate(int = as.numeric(str_remove_all(int, "[(c]")),
...
so_run_comp <-so_run%>%
  group_by(run,Chain,Iteration)%>%
  expand(int,slope,overall_int,overall_slope,age = unique(me_model_data$age))%>%
  mutate(so_ci = int + slope * age,
         so = overall_int + overall_slope * age) %>%
  group_by(run,Chain, age) %>%
  summarise(so = estimate_mode(so), 
            so_uci = hdi_upper(so_ci),
            so_lci = hdi_lower(so_ci))

ggplot(so_run_comp, aes(x = age+2, y= so, fill = run, group = paste(run,Chain)))+
  geom_line(size = .3, aes(col = run))+
  geom_ribbon(aes(ymin=so_lci, ymax = so_uci), alpha = .1)+
...
inv_model_data <- item_params_summary%>%
  ...
  expand(item, int,slope,age = unique(me_model_data$age- min(me_model_data$age)))%>%
...
novel_data <- read_csv("../data/child_spin_novelty.csv")%>%
...
```{r}
ggplot(comb_model_predictions, aes(x=age,y = me_effect, col = item))+
  geom_hline(yintercept = 0.5, lty=2)+
  geom_line(size = 1)+
