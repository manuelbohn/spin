...
var all_log_likes = map(function(row) { 
  ...
  return {
    ...
    priorOnly: Math.log(row.correct == 1 ? row.prior : 1 - row.prior)
  }
...
query.add(["log_like", "pragmatics_global", "NA"], ["NA", log_likelihood_prag_global])
query.add(["log_like", "flat_prior", "NA"], ["NA", log_likelihood_flat])
query.add(["log_like", "prior_only", "NA"], ["NA", log_likelihood_prior_only])
...
saveRDS(sem_know_model, "../saves/sem_know_model_variant3_100k_model_comparison.rds")

sem_know_model <- readRDS("../saves/sem_know_model_variant3_100k_model_comparison.rds")
