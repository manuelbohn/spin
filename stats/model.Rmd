...
var subjects = levels(priorData, "subid")
...
var model  = function(){
  ...
  var subject_sigma = uniformDrift({a: 0, b:3, width: 0.2}) 

  var sampleSub = function(age){
    return gaussianDrift({ mu: age, sigma: subject_sigma, width: 0.1 })
  }

  foreach(function(subid){
      var subjectData = _.filter(priorData, {subid: subid})

      //display(subjects)

      var subj_age = subjectData[0].age

      var subjectPrior = sampleSub(subj_age)

      display(subjectPrior)
...
plot_model_pred_prag_free <- model_pred_prag_free%>%
