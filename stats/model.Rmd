...
# # load this for model parameters with wider prior on so_slope (0:3)
# sem_know_model <- readRDS("../saves/sem_know_new_model_2.rds")

# # load this for model parameters with wider prior on so_slope (0:3) (second run)
# sem_know_model <- readRDS("../saves/sem_know_new_model_2_2.rds")%>%
#   filter(Chain != 2)

# # load this for model parameters with even wider prior on so_slope (0:4)
# sem_know_model <- readRDS("../saves/sem_know_new_model_3.rds")

# # load this for model parameters with wider prior on so_slope (0:4), so_int, as well as on global paramters of semantic knowledge
sem_know_model <- readRDS("../saves/sem_know_new_model_4.rds")%>%
  filter(Chain !=9)

 
 
#saveRDS(sem_know_model, "../saves/sem_know_new_model_4.rds")
...
var sampleItemParameters = function() { 
  return {
      intercept: gaussianDrift({
        mu: globalLineParameters.intercept, 
        sigma: itemVariability.intercept, 
...
var subject_sigma = uniform(0, 1) 

var sampleLinguisticCompetence = function(age){
  return gaussianDrift({ mu: age, sigma: subject_sigma, width: 0.5 })
}


// [ carrot, duck, bread, ...]  (slope and intercept)
var all_item_parameters = repeat(n_items, sampleItemParameters)
// [s1, s2, s3, ...] (intercept)
var all_subject_parameters = map(sampleLinguisticCompetence, subjects)

var sem_knowledge = logistic(item.intercept +  item.slope * linguistic_competence)
```
...
var model  = function(){
  ...
  foreach(function(subid){
      ...
      var speakerOptimality = speakerOptimalityParameters.intercept  + speakerOptimalityParameters.slope * subj_linguistic_competence
